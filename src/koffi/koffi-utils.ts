...
const KeyboardHookProc = koffi.proto(
...
export function registerKeyboardHookCallback(
  callback: (nCode: number, wParam: number, lParam: bigint) => bigint,
): ReturnType<typeof koffi.register> {
  return koffi.register(callback, koffi.pointer(KeyboardHookProc));
}

export function unregisterCallback(
  cb: ReturnType<typeof koffi.register>,
): void {
  koffi.unregister(cb);
}
